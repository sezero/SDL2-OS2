...
static HWND _makeDlg(const SDL_MessageBoxData *messageboxdata)
{
    ...
    ULONG               cbTitle  = (pszTitle == NULL)? 1 : (strlen(pszTitle) + 1);
    PSZ                 pszText  = OS2_UTF8ToSys(messageboxdata->message);
    ULONG               cbText   = (pszText  == NULL)? 1 : (strlen(pszText) + 1);
    ...
    if (pszTitle != NULL) {
        memcpy(pcDlgData, pszTitle, cbTitle);
    } else {
        *pcDlgData = '\0';
    }
    ...
    *((PULONG)pcDlgData) = FCF_TITLEBAR | FCF_SYSMENU;
    ...
    if (pszText != NULL) {
        memcpy(pcDlgData, pszText, cbText);
    } else {
        *pcDlgData = '\0';
    }
    pcDlgData += pDlgItem->cchText;
    ...
    *((PUSHORT)pcDlgData) = ((messageboxdata->flags & SDL_MESSAGEBOX_ERROR) != 0)?
    ...
    for (ulIdx = 0; ulIdx < cSDLBtnData; ulIdx++) {
        ...
        if (pszBtnText != NULL) {
            memcpy(pcDlgData, pszBtnText, cbBtnText);
        } else {
            *pcDlgData = '\0';
        }
        pcDlgData += pDlgItem->cchText;
