...
static void OS2_DetectDevices(void)
{
    ...
    memset(&stMCISysInfo, 0, sizeof(stMCISysInfo));
    acBuf[0] = '\0';
    ...
    for (ulNumber = 1; ulNumber <= ulDevicesNum;
        ...
        SDL_AddAudioDevice(0, stLogDevice.szProductInfo, (void *)ulNumber);
        ...
        memset(&stMCIOpen, 0, sizeof(stMCIOpen));
        ...
        memset(&stDevCapsParams, 0, sizeof(stDevCapsParams));
        ...
        if (LOUSHORT(ulRC) != MCIERR_SUCCESS) {
            ...
            if (stDevCapsParams.ulReturn) {
                SDL_AddAudioDevice(1, stLogDevice.szProductInfo, (void *)(ulNumber | 0x80000000));
            }
        }

        /* close the audio device, we are done querying its capabilities */
        memset(&stMCIGenericParams, 0, sizeof(stMCIGenericParams));
...
static void OS2_FlushCapture(_THIS)
{
    ...
    for (ulIdx = 0; ulIdx < pAData->cMixBuffers; ulIdx++) {
        ...
        memset(((PMCI_MIX_BUFFER)pAData->aMixBuffers)[ulIdx].pBuffer,
