...
mad_openFileRW(SDL_RWops *src, SDL_AudioSpec *mixer, int freesrc)
{
  ...
  if (mp3_mad) {
    mp3_mad->mp3file.src = src;
    mp3_mad->mp3file.start = 0;
    mp3_mad->mp3file.pos = 0;
    mp3_mad->mp3file.length = SDL_RWsize(src);
...
mad_closeFile(mad_data *mp3_mad)
{
  ...
  if (mp3_mad->freesrc) {
    SDL_RWclose(mp3_mad->mp3file.src);
  }
...
mad_seek(mad_data *mp3_mad, double position) {
  ...
  if (mad_timer_compare(mp3_mad->next_frame_start, target) > 0) {
    ...
    SDL_memset(mp3_mad->input_buffer, 0, sizeof(mp3_mad->input_buffer));
  }
