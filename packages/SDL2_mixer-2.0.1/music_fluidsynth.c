...
static FluidSynthMidiSong *fluidsynth_loadsong_common(SDL_RWops *src)
{
    ...
    rw_offset = SDL_RWtell(src);
    SDL_RWseek(src, 0, RW_SEEK_END);
    rw_size = (size_t)(SDL_RWtell(src) - rw_offset);
    SDL_RWseek(src, rw_offset, RW_SEEK_SET);
    ...
    if(SDL_RWread(src, rw_mem, rw_size, 1) != 1) {
        SDL_free(rw_mem);
...
FluidSynthMidiSong *fluidsynth_loadsong_RW(SDL_RWops *src, int freesrc)
{
    ...
    song = fluidsynth_loadsong_common(src);
    if (song && freesrc) {
        SDL_RWclose(src);
    }
    return song;
...
static void fluid_get_samples(FluidSynthMidiSong *song)
{
    ...
    cvt = &song->convert;
    SDL_memcpy(cvt->buf, data, CVT_BUFSIZE);
    if (cvt->needed) {
        cvt->len = CVT_BUFSIZE;
...
int fluidsynth_playsome(FluidSynthMidiSong *song, Uint8 *dest, int len)
{
    ...
    while (len > 0 && song->playing) {
        ...
        SDL_memcpy(dest, song->snd_available, mixable);
