...
static BOOL IT_Load(BOOL curious)
{
	...
	if((mh->cwt<=0x219)&&(mh->cwt>=0x217))
		of.modtype=_mm_strdup(IT_Version[mh->cmwt<0x214?4:5]);
	else if (mh->cwt>=0x215)
		of.modtype=_mm_strdup(IT_Version[mh->cmwt<0x214?2:3]);
	else {
		of.modtype = _mm_strdup(IT_Version[mh->cmwt<0x214?0:1]);
